# SystemPrompt 统一为 branded `string[]` 重构实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `systemPrompt` 统一为 `SystemPrompt` branded type（`readonly string[]`），修复 `stream-assistant.ts` 中 `config.systemPrompt` 被忽略的 bug，简化 Anthropic provider 逻辑，并将 `systemPrompt` 从 `AgentOptions.initialState` 移到顶层。
...
| `examples/debug-agent-chat.ts` | 修改 | 同上 |
...
- [ ] **Step 1: 导入 `asSystemPrompt` 并修改返回类型**
  ...
  注意：`SystemPrompt` 需要从 `../../core/ai/index.js` 导入，而不是 `./types.js`。实际上 `./types.js` 中没有 `SystemPrompt`。所以应该改为：
  ```typescript
...
- [ ] **Step 1: 更新 `agent-loop.test.ts`**
  ...
  将所有 `systemPrompt: "test"` 改为 `systemPrompt: asSystemPrompt(["test"])`（约 8 处）。
...
- [ ] **Step 3: 更新 `examples/agent-math.ts`**
  ...
  将 `initialState` 中的 `systemPrompt` 移出，改为顶层：
  ```typescript
  const agent = new Agent({
